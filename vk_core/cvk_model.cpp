...
CvkModel::CvkModel(CvkDevice &device, const std::vector<Vertex> &vertices) : cvkDevice{device} {
  createVertexBuffers(vertices);
}

CvkModel::~CvkModel() {
  vkDestroyBuffer(cvkDevice.device(), vertexBuffer, nullptr);
  vkFreeMemory(cvkDevice.device(), vertexBufferMemory, nullptr);
}
...
void CvkModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
  ...
  void *data;
  vkMapMemory(cvkDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
  memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
  vkUnmapMemory(cvkDevice.device(), vertexBufferMemory);
}
